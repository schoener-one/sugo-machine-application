...
class Parser:
    ...
    def _parse_inbound(self, inbound) -> Inbound:
        requests = list(
            map(lambda message: Parser._parse_message(message), inbound['requests'])) if 'requests' in inbound else list()
        ...
        return Inbound(requests, notifications)
        ...
        for component_name, component in self._components.items():
            for notification in component.inbound.notifications:
                ...
                if not chunks[0] in self._components.keys():
                    raise ParseException(
                        f"invalid request {notification} from {component_name} refers to unknown component {chunks[0]}")
                if not self._component_has_notification(chunks[0], chunks[1]):
                    raise ParseException(
                        f"invalid request {notification} from {component_name} has no notification {chunks[1]} in {chunks[0]}")
            for request in component.inbound.requests:
                if '.' in request.name:
                    raise ParseException(
                        f"invalid request {request.name} from {component_name} has separator")
