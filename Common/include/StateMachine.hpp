...
public:
	...
	struct Transition
	{
		Transition(StateT _state, StateT _next, EventT _event,
				Action _action = nullptr, Guard _guard = nullptr)
		: state(_state)
		, next(_next)
		, event(_event)
		, action(_action)
	...
	void push(const EventT &event, Action transitionNotFound = nullptr)
	{
		...
		if (lock.owns_lock()) // Only if holding the lock, otherwise don't block!
		{
			...
			while(success && (!m_queueEvents.empty()))
			{
				...
				if (!success)
				{
					LOG(error) << "Transition for event '" << event
							   << "' not found in state '" << m_state << "'";
