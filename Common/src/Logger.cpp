...
void Logger::reinit(logging::trivial::severity_level severity)
{
	...
	logging::core::get()->add_thread_attribute("File",
			logging::attributes::mutable_constant<std::string>(""));
	logging::core::get()->add_thread_attribute("Line",
			logging::attributes::mutable_constant<int>(0));
	logging::core::get()->add_thread_attribute("Function",
			logging::attributes::mutable_constant<std::string>(""));
}
void Logger::init(logging::trivial::severity_level severity)
{
	reinit(severity);

	// console sink
	auto consoleSink = logging::add_console_log(std::clog);
	consoleSink->set_formatter(expr::stream
		<< "["
